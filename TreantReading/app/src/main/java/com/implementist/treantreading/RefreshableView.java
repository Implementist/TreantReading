...
public class RefreshableView extends ViewGroup implements NestedScrollingParent,
    ...
    private static final String LOG_TAG = "RefreshableView";
    ...
    private void postSetRefreshing(boolean refreshing) {
        if (refreshing) {
            if (!mRefreshing) {
                ...
                mPullHandler.onRefreshing(RefreshableView.this);
            ...
            if (mRefreshing) {
                ...
                mRefreshing = refreshing;
                mPullHandler.onPullRefreshComplete(RefreshableView.this);
    ...
    public void addView(View child) {
        if (getChildCount() > 1) {
            throw new IllegalStateException("RefreshableView can host only one direct child");
        }
        super.addView(child);
    }
...
//                    final float overscrollTop = (y - mInitialMotionY) * DRAG_RATE;
    ...
    private void finishSpinner() {
        int offsetToRefresh = mPullHandler.getConfig().offsetToRefresh(RefreshableView.this, mHeaderView);
    ...
    private void touchMove(int dy) {
        ...
        int scrollY = scrollerHelper.getOffsetY();
        int totalDistance = mPullHandler.getConfig().totalDistance(RefreshableView.this, mHeaderView);
        int currentDistance = -scrollY;
        if (currentDistance + (-dy) < totalDistance) {
            ...
            dy = mPullHandler.getConfig().dispatchTouchMove(RefreshableView.this, dy, currentDistance, totalDistance);
    ...
    private class AllScroller extends ScrollerHelper {
        ...
        public void startScroll(int startX, int startY, int dx, int dy) {
            ...
            ViewCompat.postInvalidateOnAnimation(RefreshableView.this);
        }
...
// Clamp values if at the limits and record
            ...
            if (deltaY != 0) {
                ...
                mPullHandler.onPositionChange(RefreshableView.this, mStatus, deltaY, currentDistance);
            }
            if (!mScroller.isFinished() && mScroller.getFinalY() != mScroller.getCurrY()) {
                ViewCompat.postInvalidateOnAnimation(RefreshableView.this);
            }
            if (DEBUG)
                Log.d("Test", " bottom:" + mHeaderView.getBottom() + " mHeaderView.getTop:" + mHeaderView.getTop() + " scrollY:" + getScrollY() + " newScrollY:" + newScrollY + " deltaY:" + deltaY + " finalY:" + mScroller.getFinalY());
        }
    }
